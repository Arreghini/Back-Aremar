...
const partialRefundController = async (reservationId, newCheckOutDate, newCheckInDate) => {
  try {
    ...
    const originalDays = Math.ceil((new Date(reservation.checkOut) - new Date(reservation.checkIn)) / (1000 * 60 * 60 * 24));
    const newDays = Math.ceil((new Date(newCheckOutDate) - new Date(reservation.checkIn)) / (1000 * 60 * 60 * 24));
