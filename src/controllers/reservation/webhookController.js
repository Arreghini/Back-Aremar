...
async function actualizarReserva(
  ...
  if (Math.abs(totalPaid - montoAAgregar) > tolerancia) {
    throw new Error(
      `Monto pagado (${totalPaid}) no coincide con el esperado (${montoAAgregar})`
    );
  }
  ...
  const mensaje =
    paymentType === 'deposit' ? 'Pago de seña recibido' : 'Pago total recibido';
  ...
  try {
    ...
    if (topic === 'merchant_order') {
      ...
      try {
        ...
        console.error(
          'Error al parsear external_reference:',
      ...
      console.log(
        `Procesando webhook para reserva ${reservationId}, tipo de pago: ${paymentType}`
      );
