...
const createRoomController = async (data) => {
  try {
    ...
    console.log(
      'Datos recibidos en el controlador de habitación:',
    ...
    if (roomDetails) {
      try {
        ...
        throw new Error(
          'Los detalles de la habitación no tienen un formato JSON válido'
        );
      }
    }
    ...
    if (parsedRoomDetails) {
      try {
        ...
        throw new Error(
          `Error al crear los detalles de la habitación: ${detailError.message}`
        );
      }
    }
    ...
    console.log(
      'Datos finales para crear habitación:',
